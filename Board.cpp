...
bool Board::isConnected(City& city1, City&city2){
    if(connections.at(city1).count(city2)) {return true;}
    else {return false;}
}
...
std::ostream& pandemic::operator<<(std::ostream& os, const Board& br) {
    
    for(auto& k: br.diseasCubes) {
        os << "\t" << cityToString(k.first) << ": \t\t" << k.second << std::endl;
    }
    os  << "cure finded for the colors:" << std::endl;
    for(int c = 0; c < 4; c++) {
        os << "\t" << colorToString(c) << ":\t\t" << br.cured[c] << std::endl;
    }
    os << "research stations:" << std::endl;
    for(auto& str: br.research_stations) {
        os << "\t" << cityToString(str) << std::endl;
    }
    return os;
}
